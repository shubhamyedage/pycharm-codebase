...
def get_income_tree():
    ...
    tree.create_node(con.NET_INCOME, get_string(con.NET_INCOME), parent=get_string(con.INCOME_BEFORE_TAXES),
                     data={'val': -1})
    tree.create_node(con.TOTAL_OUTSTANDING_SHARES, get_string(con.TOTAL_OUTSTANDING_SHARES), parent=get_string(con.NET_INCOME),
                     data={'val': -1})
    tree.create_node(con.EARNING_PER_SHARE, get_string(con.EARNING_PER_SHARE),
                     parent=get_string(con.NET_INCOME),
                     data={'val': -1})
